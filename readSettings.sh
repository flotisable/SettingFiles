...
for target in $(mapKeys "$targetMapName"); do

  if [ -n "$(mapFind "$targetMapName" "$target")" ]; then

    continue

  fi

  default=$(mapFind "defaults" "$target")

  if [ $(isMap $default) -eq 1 ]; then

    default=$(mapFind "$default" "$os")

  fi

  mapSet "$targetMapName" "$target" "$default"
